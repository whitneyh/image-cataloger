...
public class StandardWriterStrategy extends WriterStrategySupport {
    ...
    @Override
    public void write(File outputBaseDir, ImageMetadata imageMetadata) throws IOException {
        ...
        if (originalDate != null) {
            ...
            String fileSuffix = FileUtil.getNormalizedSuffix(imageMetadata.getFile());
            int imageNumber = calculateImageNumber(outputPath, fileBaseName);
            File outputFilePath = calculateFilePath(outputPath, fileBaseName, fileSuffix, imageNumber);
    ...
    private File calculatePath(File outputBaseDir, LocalDateTime originalDate) {
        ...
        String month = originalDate.getMonth().getDisplayName(TextStyle.FULL, Locale.US);
        String pathString = String.format("%4d%sFamily%s%s", year, File.separator, File.separator, month);
        return new File(outputBaseDir, pathString);
    }

    String calculateFileBase(LocalDateTime originalDate) {
        int year = originalDate.getYear();
        int month = originalDate.getMonth().getValue();
    ...
    private File calculateFilePath(File outputPath, String fileBaseName,
        ...
        String fileName = String.format("%s_%04d.%s", fileBaseName, imageNumber, fileSuffix);
        return new File(outputPath, fileName);
    }

    private void copyFile(File inputFilePath, File outputFilePath)
            throws IOException {
        Files.copy(inputFilePath.toPath(), outputFilePath.toPath(), REPLACE_EXISTING, COPY_ATTRIBUTES);
    }
